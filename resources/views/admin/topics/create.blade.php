    ...
    <div class="space-y-6" id="topicStudio" data-preview-url="{{ route('admin.topics.preview') }}">
        ...
        <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)]">
            <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-200 space-y-6">
                    <form method="POST" action="{{ route('admin.topics.store') }}" class="space-y-6" id="topicForm">
                        @csrf

                        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                            <div class="text-sm text-gray-500">
                                {{ __('Paste a JSON payload from ChatGPT, auto-fill the fields, preview, then save.') }}
                            </div>
                        ...
                        <div>
                            ...
                            <textarea id="data" name="data" rows="16" class="mt-1 font-mono text-sm block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="{{ __('Paste the JSON payload here') }}" required>{{ old('data') }}</textarea>
                        ...
                        <div class="grid gap-6 md:grid-cols-2">
                            <div>
                                ...
                                <input id="title" name="title" type="text" value="{{ old('title') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" autocomplete="off" required>
                                <x-input-error :messages="$errors->get('title')" class="mt-2" />
                            </div>

                            <div>
                                <label for="slug" class="block text-sm font-medium text-gray-700">{{ __('Slug') }}</label>
                                <input id="slug" name="slug" type="text" value="{{ old('slug') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" autocomplete="off" required>
                                <x-input-error :messages="$errors->get('slug')" class="mt-2" />
                            </div>

                            <div>
                                <label for="subtitle" class="block text-sm font-medium text-gray-700">{{ __('Subtitle') }}</label>
                                <input id="subtitle" name="subtitle" type="text" value="{{ old('subtitle') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            <div>
                                ...
                                <textarea id="summary" name="summary" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('summary') }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea id="meta" name="meta" rows="6" class="mt-1 font-mono text-sm block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('meta') }}</textarea>
                            <x-input-error :messages="$errors->get('meta')" class="mt-2" />
                        </div>

                        <div class="flex items-center justify-between">
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="publish" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" @checked(old('publish', false))>
                                <span class="ms-2 text-sm text-gray-700">{{ __('Publish immediately') }}</span>
                            </label>
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">{{ __('Use “Preview” to render the current JSON with the selected template.') }}</p>
                    </div>
