...
<x-admin-layout :page-title="$pageTitle" :page-description="$pageDescription">
        ...
        <div class="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $stats['in_review'] }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $stats['drafts'] }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-6">
        <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <form method="GET" action="{{ route('admin.topics.index') }}" class="grid gap-4 md:grid-cols-4">
                    <div class="md:col-span-2">
                        <label for="q" class="block text-sm font-medium text-gray-700">{{ __('Search') }}</label>
                        <input id="q" name="q" type="text" value="{{ $filters['q'] }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="{{ __('Search by title or slug') }}">
                    </div>
                    <div>
        ...
        <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-4" id="bulk-actions-form" style="display: none;">
            <form action="{{ route('admin.topics.bulk-action') }}" method="POST" id="bulk-form">
                @csrf
                <div class="flex flex-wrap items-center gap-4">
                    <div class="flex items-center space-x-2">
                        <span class="text-sm font-medium text-gray-700">{{ __('With selected:') }}</span>
                    ...
                    <select name="action" id="bulk-action" class="rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <option value="review">{{ __('Move to Review') }}</option>
                        <option value="change_category">{{ __('Change Category') }}</option>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Title') }}</th>
                                <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Slug') }}</th>
                                ...
                                <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Updated') }}</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 text-sm">
                            @forelse ($topics as $topic)
                                ...
                                <tr>
                                    <td class="px-3 py-3">
                                        <input type="checkbox" name="topics[]" value="{{ $topic->id }}" class="topic-checkbox rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    </td>
                                    <td class="px-3 py-3">
                                        <div class="font-medium text-gray-900">{{ $topic->title }}</div>
                                        <div class="text-gray-500 text-xs">{{ $topic->template?->name ?? __('Default template') }}</div>
                                    </td>
                                    <td class="px-3 py-3 text-gray-500">{{ $topic->slug }}</td>
                                    <td class="px-3 py-3 text-gray-500">{{ $topic->category?->name ?? __('Uncategorized') }}</td>
                                    <td class="px-3 py-3">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $statusClasses }}">
                                            {{ ucfirst($topic->status) }}
                                        </span>
                                    </td>
                                    <td class="px-3 py-3 text-gray-500">{{ $topic->updated_at->diffForHumans() }}</td>
                                    <td class="px-3 py-3 text-right space-x-2">
                                        <a href="{{ route('topics.show', $topic) }}" class="text-green-600 hover:text-green-800">{{ __('Preview') }}</a>
                                        <a href="{{ route('admin.topics.wysiwyg', $topic) }}" class="text-purple-600 hover:text-purple-800">{{ __('WYSIWYG') }}</a>
                                        <a href="{{ route('admin.topics.edit', $topic) }}" class="text-indigo-600 hover:text-indigo-800">{{ __('Edit') }}</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="7" class="px-3 py-6 text-center text-gray-500">{{ __('No topics found.') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
