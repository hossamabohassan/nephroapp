                    ...
                    @forelse($menuItems as $menuItem)
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-900">
                                <span class="bg-slate-100 px-2 py-1 rounded text-xs font-medium">{{ $menuItem->order }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    @endif
                                    <div>
                                        <div class="text-sm font-medium text-slate-900">{{ $menuItem->title }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-900">
                                <code class="bg-slate-100 px-2 py-1 rounded text-xs">{{ $menuItem->url }}</code>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                                {{ $menuItem->icon ?: 'default' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($menuItem->permission === 'admin')
                                    <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800">
                                        👑 Admin
                                    </span>
                                @elseif($menuItem->permission === 'editor')
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-slate-500">
                                <div class="flex flex-col items-center">
                                    ...
                                    <a href="{{ route('admin.menu-items.create') }}" class="text-blue-600 hover:text-blue-800 font-medium mt-2">Create one now</a>
                                </div>
                            </td>
                        </tr>
                    @endforelse
