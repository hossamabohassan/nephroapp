    ...
    @if(isset($menuItems) && $menuItems->count() > 0)
        <nav class="space-y-2">
            @foreach($menuItems as $menuItem)
                <a href="{{ $menuItem->url }}" 
                   class="group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-300 transform hover:scale-105 hover:shadow-lg {{ request()->is(ltrim($menuItem->url, '/')) ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-lg' : 'text-gray-700 hover:bg-gradient-to-r hover:from-gray-100 hover:to-gray-200' }}"
                   @if($menuItem->opens_in_new_tab) target="_blank" @endif>
                    
                    @if($menuItem->icon === 'home')
                        <svg class="w-5 h-5 mr-3 {{ request()->is(ltrim($menuItem->url, '/')) ? 'text-white' : 'text-blue-500 group-hover:text-blue-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
                        </svg>
                    @elseif($menuItem->icon === 'book')
                        <svg class="w-5 h-5 mr-3 {{ request()->is(ltrim($menuItem->url, '/')) ? 'text-white' : 'text-green-500 group-hover:text-green-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
                        </svg>
                    @elseif($menuItem->icon === 'info')
                        <svg class="w-5 h-5 mr-3 {{ request()->is(ltrim($menuItem->url, '/')) ? 'text-white' : 'text-yellow-500 group-hover:text-yellow-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                    @elseif($menuItem->icon === 'user')
                        <svg class="w-5 h-5 mr-3 {{ request()->is(ltrim($menuItem->url, '/')) ? 'text-white' : 'text-purple-500 group-hover:text-purple-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                        </svg>
                    @elseif($menuItem->icon === 'settings')
                        <svg class="w-5 h-5 mr-3 {{ request()->is(ltrim($menuItem->url, '/')) ? 'text-white' : 'text-gray-500 group-hover:text-gray-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @elseif($menuItem->icon === 'mail')
                        <svg class="w-5 h-5 mr-3 {{ request()->is(ltrim($menuItem->url, '/')) ? 'text-white' : 'text-red-500 group-hover:text-red-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if($menuItem->opens_in_new_tab)
                        <svg class="w-4 h-4 {{ request()->is(ltrim($menuItem->url, '/')) ? 'text-white' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
