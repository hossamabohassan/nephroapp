...
<x-admin-layout :page-title="$pageTitle" :page-description="$pageDescription">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 card-hover">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-slate-900">{{ $topicCounts['total'] }}</p>
                    <div class="flex items-center space-x-4 mt-2 text-xs">
                        <span class="text-green-600">✓ {{ $topicCounts['published'] }} Published</span>
                        <span class="text-yellow-600">⏳ {{ $topicCounts['drafts'] }} Drafts</span>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 card-hover">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-slate-900">{{ $userCounts['total'] }}</p>
                    <div class="flex items-center space-x-4 mt-2 text-xs">
                        <span class="text-green-600">✓ {{ $userCounts['active'] }} Active</span>
                        <span class="text-purple-600">👑 {{ $userCounts['admins'] }} Admins</span>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 card-hover">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-slate-900">{{ \App\Models\Category::count() }}</p>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 card-hover">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-slate-900">{{ $recentActivity->count() }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <a href="{{ route('admin.topics.create') }}" class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white card-hover group">
        ...
        <a href="{{ route('admin.categories.index') }}" class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white card-hover group">
            <div class="flex items-center justify-between mb-4">
                <div class="w-12 h-12 bg-white/20 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/>
        ...
        <a href="{{ route('admin.content.index') }}" class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-6 text-white card-hover group">
            <div class="flex items-center justify-between mb-4">
                <div class="w-12 h-12 bg-white/20 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('admin.topics.index') }}" class="text-sm text-blue-600 hover:text-blue-800 font-medium">View all →</a>
            </div>
            ...
            <div class="space-y-4">
                @forelse ($recentTopics as $topic)
                    <div class="flex items-center justify-between p-4 border border-slate-100 rounded-lg hover:border-slate-200 transition-colors">
                        <div class="flex-1">
                            <h4 class="font-medium text-slate-900 truncate">{{ $topic->title }}</h4>
                            <div class="flex items-center space-x-3 mt-1">
                                ...
                                <span class="text-xs text-slate-500">{{ $topic->updated_at->diffForHumans() }}</span>
                            </div>
                        </div>
                        <div class="flex items-center space-x-2 ml-4">
                            <a href="{{ route('topics.show', $topic) }}" class="p-2 text-slate-400 hover:text-blue-600 transition-colors" title="Preview">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                </svg>
                            </a>
                            <a href="{{ route('admin.topics.edit', $topic) }}" class="p-2 text-slate-400 hover:text-blue-600 transition-colors" title="Edit">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                </svg>
                            </a>
                ...
                @empty
                    <div class="text-center py-8">
                        ...
                        <a href="{{ route('admin.topics.create') }}" class="text-sm text-blue-600 hover:text-blue-800 font-medium">Create your first topic →</a>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('admin.activity.index') }}" class="text-sm text-blue-600 hover:text-blue-800 font-medium">View all →</a>
            </div>
                ...
                @forelse ($recentActivity as $entry)
                    <div class="flex items-start space-x-3 p-4 border border-slate-100 rounded-lg">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-slate-500 mt-1">{{ $entry->created_at->diffForHumans() }}</p>
                        </div>
                    </div>
                @empty
                    <div class="text-center py-8">
